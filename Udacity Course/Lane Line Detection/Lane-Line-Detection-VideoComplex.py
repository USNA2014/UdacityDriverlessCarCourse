      ...
      for line in lines:
          for x1,y1,x2,y2 in line:
            ...
            if ((y2-y1)/(x2-x1))<0:
              ...
              lower_x.append(x1-80)
              upper_x.append(x2)
              lower_x_avg= int(np.average(lower_x))
              upper_x_avg= int(np.average(upper_x))
              lower_y.append(y1+70)
              upper_y.append(y2)
              lower_y_avg= int(np.average(lower_y))
              upper_y_avg= int(np.average(upper_y))
            
              cv2.line(line_image,(lower_x_avg,lower_y_avg),(upper_x_avg,upper_y_avg),(0,0,255),10)
              
            else:

              lower_x2.append(x1)
              upper_x2.append(x2+100)
              lower_x_avg2= int(np.average(lower_x2))
              upper_x_avg2= int(np.average(upper_x2))
              lower_y2.append(y1)
              upper_y2.append(y2+70)
              lower_y_avg2= int(np.average(lower_y2))
              upper_y_avg2= int(np.average(upper_y2))
              
              cv2.line(line_image,(lower_x_avg2,lower_y_avg2),(upper_x_avg2,upper_y_avg2),(0,0,255),10)
